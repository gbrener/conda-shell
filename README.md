...
>    - Syntactically, `conda-shell`'s use of the [shebang line](https://en.wikipedia.org/wiki/Shebang_(Unix)) - borrowed from `nix-shell`'s syntax - is more terse and reuses the CLI from the `conda install` command. This avoids the need to invent and maintain a new YAML-based syntax for declaring package dependencies
